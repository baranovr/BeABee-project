...
export const TotalEarning: React.FC = () => {
  ...
  useEffect(() => {
    userId && getTotalEarning(userId, CurrencyTypeEnum.UAN).then((res) => setTotalEarning(res));
  }, [userId]);
      ...
      <BaseRow gutter={[14, 14]}>
        ...
        <BaseCol span={24}>
          <BaseRow wrap={false} justify="space-between" gutter={[20, 20]}>
            <BaseCol>
              <S.Text>
                {getCurrencyPrice(formatNumberWithCommas(totalEarning?.total ?? 0), CurrencyTypeEnum.UAN)}
              </S.Text>
            </BaseCol>
